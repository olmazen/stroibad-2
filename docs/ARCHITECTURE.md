...
Общие `style.css`, `site.js`, `leads.js`, `ad3d.js` и `fw3d.js`, а также отдельный `kp.js`, подключаются с версией из первых 12 символов SHA-256 собственного содержимого. `tools/sync-runtime-assets.mjs` контролируемо подключает lead-модуль после runtime bootstrap, обновляет 303 стандартные страницы и точечно `/kp/`; `npm run build` отклоняет отсутствующую или устаревшую версию. Это исключает ситуацию, когда production уже обновлён, а браузер ещё месяц использует закэшированный старый runtime.
...
Текущие FormSubmit и Google Apps Script сохранены только как переходный текстовый транспорт. Только строго разобранный ответ FormSubmit считается подтверждаемым; relay в Telegram запускается лишь после этого как вторичный best-effort канал. AJAX-контракт вложений у FormSubmit не опубликован, поэтому они не включаются. FormSubmit указывает хранение отправок 30 дней, поэтому эта схема не является финальным решением для заявленной российской локализации данных: [документация провайдера](https://formsubmit.co/documentation). Следующий отдельный этап — один контролируемый endpoint в российском регионе, серверная проверка, идемпотентность, антиспам, политика хранения/удаления и юридическое согласование.
