...
Общие `style.css`, `site.js`, `leads.js`, `ad3d.js` и `fw3d.js` подключаются с версией из первых 12 символов SHA-256 собственного содержимого. `tools/sync-runtime-assets.mjs` контролируемо подключает lead-модуль после runtime bootstrap и меняет только соответствующие `href/src` в 303 стандартных страницах; `npm run build` отклоняет отсутствующую или устаревшую версию. Это исключает ситуацию, когда production уже обновлён, а браузер ещё месяц использует закэшированный старый runtime.
...
- success показывается только после HTTP `2xx`, JSON и явного признака принятия;
