...
function cleanDescription(product) {
  ...
  if (!raw) return 'Уличная скамейка для благоустройства общественных пространств. Производим под проект, подбираем цвет металла, древесину и комплектацию под объект.';
...
<header id="siteHeader"><div class="container hdr">
  ...
  <nav class="main" id="nav">
    <div class="navitem"><a href="${prefix}maf/index.html">Каталог</a>
      <div class="dropdown">
        <a class="dd-item" href="${prefix}maf/index.html"><span class="dd-ico"><svg viewBox="0 0 24 24"><path d="M3 11l2-3h14l2 3M3 11h18M5 11v6M19 11v6M3 17h3M18 17h3"/></svg></span><span class="dd-tx"><b>Малые архитектурные формы</b><small>Скамейки, качели, урны</small></span></a>
...
<footer>
  ...
  <div class="container">
    <div class="foot-grid">
      ...
      <div><div class="foot-col-h">Продукция</div><a href="${prefix}maf/skamejki/index.html">Скамейки</a><a href="${prefix}maf/kacheli/index.html">Качели</a><a href="${prefix}maf/urny/index.html">Урны</a><a href="${prefix}maf/veloparkovki/index.html">Велопарковки</a><a href="${prefix}ograzhdeniya/index.html">Ограждения</a></div>
...
<link rel="stylesheet" href="${cssPrefix}assets/css/style.css?v=skamejki-ai-1">
</head>
<body>
${body}
...
function card(product, prefix = '../../') {
  const img = `${prefix}assets/img/maf/skamejki/${product.sku}/main.webp`;
  return `<a class="model-card reveal" href="${prefix}maf/skamejki/${product.pageSlug}/index.html">
...
<section>
  <div class="container">
    ...
    <div class="sec-head reveal" style="margin-top:24px"><div><h2>Выберите модель скамейки</h2></div><p class="lead-wide">Все позиции взяты из согласованной номенклатуры. Фото можно автоматически обновлять из AI-генератора: страницы и карточки подхватят новые кадры после публикации.</p></div>
    <div class="model-grid">${products.map((p) => card(p, prefix)).join('\n')}</div>
  </div>
</section>
<section class="dev" style="padding:72px 0"><div class="container"><div class="split">
  <div class="reveal"><div class="dim on-dark"><span class="tick"></span>Под объект</div><h2 style="margin:22px 0 16px;font-size:clamp(24px,3vw,36px)">Комплектуем двор скамейками, урнами, качелями и ограждениями в одном стиле</h2><p style="color:#B5BAC0;font-size:16px;max-width:520px">Подбираем серию под дизайн-код территории: единый RAL, древесина, закладные, крепёж и поставка партиями под график благоустройства.</p><button class="btn btn-primary" style="margin-top:24px" onclick="openModal()">Запросить расчёт серии</button></div>
  <ul class="devlist reveal" style="--d:.1s"><li><span><b>${products.length} моделей.</b> От компактных лавок до модульных и радиусных решений.</span></li><li><span><b>Единый RAL.</b> Металл окрашиваем в цвет проекта и соседних МАФ.</span></li><li><span><b>Проектная поставка.</b> Считаем партию, логистику, монтажные закладные и сроки.</span></li><li><span><b>Документы.</b> Работаем с НДС, готовим паспорта изделий и спецификации.</span></li></ul>
</div></div></section>
...
function renderProduct(cfg, product, allProducts) {
  ...
  const specRows = product.specs.length ? product.specs : [['Материал', 'сталь, дерево'], ['Тип товара', 'Скамейка уличная'], ['Поставка', 'по России']];
  const description = cleanDescription(product.raw);
...
<section><div class="container"><div class="prodpage">
  ...
  <div class="pp-info">
    ...
    <div class="pp-art">Артикул ${esc(product.sku)} · скамейка для благоустройства</div>
...
<section style="padding-top:0"><div class="container"><div class="sec-head"><div><h2>Другие модели скамеек</h2></div><a class="btn btn-sm" href="${prefix}maf/${cfg.target}/index.html">Все скамейки</a></div><div class="tiles3">${others.map((p) => `<a class="tile" href="${prefix}maf/${cfg.target}/${p.pageSlug}/index.html"><div class="ph has-img r-43"><img src="${prefix}assets/img/maf/${cfg.imgTarget}/${p.sku}/main.webp" alt="${esc(p.name)}" loading="lazy"><span class="ph-label">${esc(p.shortName)}</span></div><h3>${esc(p.name)}</h3><p>${esc(p.priceText)}</p></a>`).join('')}</div></div></section>
...
function updateSitemap(siteRoot, cfg, products) {
  ...
  const urls = staticUrls.concat(products.map((p) => [`/maf/${cfg.target}/${p.pageSlug}/`, '0.6']));
...
function normalizeProducts(products, propertiesBySku, manifests, cfg) {
  ...
  return products.map((raw, idx) => {
    ...
    const shortName = product.name.replace(/^Скамейка стальная\s*/i, '').replace(/^Скамейка\s*/i, '').replace(/^Банкетка\s*/i, 'Банкетка ');
    return {
      sku,
      raw: product,
      manifest,
      ...
      cardText: cleanDescription(product).slice(0, 170),
